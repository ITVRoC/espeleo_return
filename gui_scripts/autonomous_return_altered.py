...
interface = rospy.get_param("comms_interface_enable")  # Interface used to control the communication state in simulations.
    ...
    while not rospy.is_shutdown():
        ...
        while (not lost_signal):
            ...
            if control:
                ...
                rospy.set_param('/espeleo/return_finished', True)
        ...
        if lost_signal:
            ...
            if(intermediate_stop):
                ...
                traj_msg = create_traj_msg(position_buffer[i_aux1:i_aux2])
...
# Callback that updates dynamic parameters value
def emit(config):
    rospy.loginfo('emitting')
    # Loading needed parameters from the dynamic server
    global lost_signal
    lost_signal = rospy.get_param('/espeleo/return_active')
    global size
    size = rospy.get_param('/espeleo/return_distance')
